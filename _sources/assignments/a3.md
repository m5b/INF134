...
Although there are many tools available for building a widget toolkit, you will be required to use the Javascript library for Scalable Vector Graphics called [SVG.js](https://svgjs.dev/docs/3.0/). Your widgets, therefore, will be rendered in a browser. If you haven't yet, review the week 4 lecture for an overview on how to get started.

```{note}

You are free to use Javascript to create your toolkit. However, you might also consider using TypeScript as its type system and similarities with Java are more conducive to GUI programming.
